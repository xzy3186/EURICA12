...
void rootlogon()
{
  ...
  gROOT->ProcessLine(".L macros/myfunc.C");
  gROOT->ProcessLine(".L macros/RooMyDecay.cxx+");
  gROOT->ProcessLine(".L macros/RooMyDecay_full.cxx+");
  //gROOT->ProcessLine(".L macros/fit_decay.C");
...
//  modules.push_back(install_dir+"/lib/"+"libananadeko.so"); // load each modules one by one
  ...
  modules.push_back(install_dir2+"/macro/RIBeam_h.so");
  modules.push_back(install_dir2+"/macro/RIBeamOthers_h.so");
