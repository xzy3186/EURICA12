...
int main(int argc, char* argv[]){
   ...
   Int_t imp_gc_hit;
   Int_t imp_gc_ch[84];
   Double_t imp_gc_E[84];
   Double_t imp_gc_T[84];
   Int_t imp_ab_hit;
   Int_t imp_ab_ch[84];
   Double_t imp_ab_E[84];
   Double_t imp_ab_T[84];
   ...
   sprintf(outfile,"../gamma_db/decay_new_zn84_%04d.root",runD);
   int zed_set = 30;
   int mass_set = 84;
   ...
   decay->Branch("imp_gc_hit",&imp_gc_hit,"imp_gc_hit/I");
   decay->Branch("imp_gc_ch",imp_gc_ch,"imp_gc_ch[imp_gc_hit]/I");
   decay->Branch("imp_gc_E",imp_gc_E,"imp_gc_E[imp_gc_hit]/D");
   decay->Branch("imp_gc_T",imp_gc_T,"imp_gc_T[imp_gc_hit]/D");
   decay->Branch("imp_ab_hit",&imp_ab_hit,"imp_ab_hit/I");
   decay->Branch("imp_ab_ch",imp_ab_ch,"imp_gc_ch[imp_ab_hit]/I");
   decay->Branch("imp_ab_E",imp_ab_E,"imp_gc_E[imp_ab_hit]/D");
   decay->Branch("imp_ab_T",imp_ab_T,"imp_gc_T[imp_ab_hit]/D");
   ...
   for(dEntry = 0; dEntry < dEntryMAX; dEntry++){
      ...
      imp_gc_hit = ggdecay.gc_hit;
      for(int i=0; i<ggdecay.gc_hit; i++){
         imp_gc_ch[i] = ggdecay.gc_ch[i];
         imp_gc_E[i] = ggdecay.gc_E[i];
         imp_gc_T[i] = ggdecay.gc_T[i];
      }
      imp_ab_hit = ggdecay.ab_hit;
      for(int i=0; i<ggdecay.ab_hit; i++){
         imp_ab_ch[i] = ggdecay.ab_ch[i];
         imp_ab_E[i] = ggdecay.ab_E[i];
         imp_ab_T[i] = ggdecay.ab_T[i];
      }
