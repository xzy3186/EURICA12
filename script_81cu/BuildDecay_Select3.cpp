/************************************
To find all the decay events before
given gamma-ray(s) in the same decay
chain.
************************************/
...
int main(int argc, char* argv[]){
   ...
   sprintf(decayfile,"../root/final/cu81test/decay_new%04d.root",runD);
   //sprintf(decayfile,"go4.root");
   sprintf(outfile,"../gamma_db/decay_parent_ni76_%04d.root",runD);
   int zed_set=28;
   int mass_set=76;
   int el=596;
   int er=601;
   ...
   TTree *gbg = new TTree("gbg","gbg");
   TTree *gbgbg = new TTree("gbgbg","gbgbg");
   ...
   gbg->Branch("run",&run,"run/I");
   gbg->Branch("z",&imp_z,"z/I");
   gbg->Branch("x",&imp_x,"x/I");
   gbg->Branch("y",&imp_y,"y/I");
   gbg->Branch("beta35",&beta35,"beta35/D");
   gbg->Branch("beta57",&beta57,"beta57/D");
   gbg->Branch("beta711",&beta711,"beta711/D");
   gbg->Branch("aoq37",&aoq37,"aoq37/D");
   gbg->Branch("zed",&zed,"zed/D");
   gbg->Branch("Zed",&Zed,"Zed/I");
   gbg->Branch("Mass",&Mass,"Mass/I");
   gbg->Branch("flagall",&flagall,"flagall/I");
   gbg->Branch("t",&bg_t,"t/L");
   gbg->Branch("ex",&bg_ex,"ex/I");
   gbg->Branch("ey",&bg_ey,"ey/I");
   gbg->Branch("mulx",&bg_mulx,"mulx/I");
   gbg->Branch("muly",&bg_muly,"muly/I");
   gbg->Branch("flag_side",&bg_flag_side,"flag_side/I");
   gbg->Branch("deltaz",&bg_deltaz,"deltaz/I");
   gbg->Branch("deltaxy",&bg_deltaxy,"deltaxy/D");

   gbgbg->Branch("run",&run,"run/I");
   gbgbg->Branch("z",&imp_z,"z/I");
   gbgbg->Branch("x",&imp_x,"x/I");
   gbgbg->Branch("y",&imp_y,"y/I");
   gbgbg->Branch("beta35",&beta35,"beta35/D");
   gbgbg->Branch("beta57",&beta57,"beta57/D");
   gbgbg->Branch("beta711",&beta711,"beta711/D");
   gbgbg->Branch("aoq37",&aoq37,"aoq37/D");
   gbgbg->Branch("zed",&zed,"zed/D");
   gbgbg->Branch("Zed",&Zed,"Zed/I");
   gbgbg->Branch("Mass",&Mass,"Mass/I");
   gbgbg->Branch("flagall",&flagall,"flagall/I");
   gbgbg->Branch("t",&bg_t,"t/L");
   gbgbg->Branch("ex",&bg_ex,"ex/I");
   gbgbg->Branch("ey",&bg_ey,"ey/I");
   gbgbg->Branch("mulx",&bg_mulx,"mulx/I");
   gbgbg->Branch("muly",&bg_muly,"muly/I");
   gbgbg->Branch("flag_side",&bg_flag_side,"flag_side/I");
   gbgbg->Branch("deltaz",&bg_deltaz,"deltaz/I");
   gbgbg->Branch("deltaxy",&bg_deltaxy,"deltaxy/D");
   ...
   for(dEntry = 0; dEntry < dEntryMAX; dEntry++){
      ...
      if(Zed != zed_set || Mass != mass_set){
         continue;
      }

      beta_find=-1;
      for(int i=0; i<ggdecay.beta_; i++){
         for(int j=0; j<ggdecay.beta_ab_hit[i]; j++){
            if(ggdecay.beta_deltaz[i]==0 && ggdecay.beta_deltaxy[i]<=1.5 && ggdecay.beta_flag_side[i]<2 && ggdecay.beta_ab_T[i][j]>0 && ggdecay.beta_ab_E[i][j]>el && ggdecay.beta_ab_E[i][j]<er){
               beta_find=i;
               break;
            }
         }
      }
      for(int i=0; i<beta_find; i++){
      ...
      if(beta_find != -1){
         for(int i=0; i<ggdecay.bg_; i++){
            if(ggdecay.bg_t[i] < ggdecay.beta_t[beta_find]){
               bg_find=i;
               break;
            }
         }
      }
      for(int i=bg_find; i<ggdecay.bg_; i++){
         bg_t = ggdecay.bg_t[i];
      ...
      for(int i=0; i<ggdecay.beta_; i++){
         for(int j=0; j<ggdecay.beta_ab_hit[i]; j++){
            if(ggdecay.beta_deltaz[i]==0 && ggdecay.beta_deltaxy[i]<=1.5 && ggdecay.beta_flag_side[i]<2 && ggdecay.beta_ab_T[i][j]>0 && ((ggdecay.beta_ab_E[i][j]>ell && ggdecay.beta_ab_E[i][j]<elr) || (ggdecay.beta_ab_E[i][j]>erl && ggdecay.beta_ab_E[i][j]<err))){
               beta_find=i;
               break;
            }
         }
      ...
      for(int i=0; i<beta_find; i++){
         bg_t = ggdecay.beta_t[i];
         bg_ex = ggdecay.beta_ex[i];
         bg_ey = ggdecay.beta_ey[i];
         bg_mulx = ggdecay.beta_mulx[i];
         bg_muly = ggdecay.beta_muly[i];
         bg_deltaz = ggdecay.beta_deltaz[i];
         bg_deltaxy = ggdecay.beta_deltaxy[i];
         bg_flag_side = ggdecay.beta_flag_side[i];
         gbg->Fill();
      }
