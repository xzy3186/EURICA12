...
int main(int argc, char* argv[]){
   ...
   sprintf(dssdfile,"../root/sistopper/nb115test/WASABI_EURICA%04d.root",runN);
   //sprintf(bigripsfile,"../root/bigrips/v0.nocorr/bigrips_run%04d.root",runB);
   //sprintf(outfile,"../root/sistopper/rh123test/beta_ion_%04d.root",runN);
   sprintf(bigripsfile,"../root/bigrips/v2/RIBF60-115Nb.root");
   //sprintf(outfile,"../root/sistopper/beta_ion_%04d.root",runN);
   sprintf(outfile,"../root/final/nb115test/beta_ion_%04d.root",runN);
   ...
   for(imteuro = mteuro.begin();imteuro!=mteuro.end();imteuro++){
      ...
      if(imvbe != mvbe.end()){
         ...
         ION.bigrips_aoq = brips.AOQ;
         ION.bigrips_zet = brips.ZET;
         ION.bigrips_beta = 0;
