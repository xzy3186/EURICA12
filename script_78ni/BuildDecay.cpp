...
int main(int argc, char* argv[]){
   ...
   sprintf(dssdfile,"../root/final/ni78test/beta_ion_%04d.root",runD);
   sprintf(outfile,"../root/final/ni78test/decay_new%04d.root",runD);
   ...
   for(imtsi=mtsi.begin(); imtsi!=mtsi.end(); imtsi++){
      ...
      imp->beta35 = ION.RIBeam_beta35;
      imp->beta57 = ION.RIBeam_beta57;
      imp->beta711 = ION.RIBeam_beta711;
      imp->aoq37 = ION.RIBeam_aoq37;
      imp->zed = ION.RIBeam_zed;
      imp->Zed = ION.RIBeam_Zed;
      imp->Mass = ION.RIBeam_Mass;
      imp->flagall = ION.RIBeam_flagall;
      imp->flagTSumX = ION.RIBeam_flagTSumX;
      imp->flagTOFt = ION.RIBeam_flagTOFt;
      imp->flagTOFq = ION.RIBeam_flagTOFq;
      imp->flagReaction = ION.RIBeam_flagReaction;
      imp->flagdERelation = ION.RIBeam_flagdERelation;
      imp->flagAngle = ION.RIBeam_flagAngle;
      imp->gc_hit = 0;
      for(int i=0; i<84; i++){
         imp->gc_ch[i] = -1;
         imp->gc_DGFe[i] = -1;
      ...
      if(iEntry==0){
         ...
         beta_present = imtsb;
         ts1 = imtsi->first - time_window; //100us as dead time
         imtsb = mtsb.lower_bound(ts1);
         bg_present = imtsb;
      }
      //if(ION.IfInterest()==0){ //make timestamp interval for the implantation of less neutron rich nuclei larger than 2 seconds
      //   //continue;
      //   if(ION.ts-ion_lastts[ION.RIBeam_Zed-20][ION.RIBeam_Mass-ION.RIBeam_Zed]>1e8){
      //      ion_lastts[ION.RIBeam_Zed-20][ION.RIBeam_Mass-ION.RIBeam_Zed] = ION.ts;
      //      imp->IfInterest = 1;
      //   }else{
      //      imp->IfInterest = 0;
      //      //continue;
      ...
      //cout<<imp->ts<<", "<<imp->Zed<<", "<<imp->Mass<<endl;
      decay->Fill();
   }
   rootfile->Write();
   rootfile->Close();
