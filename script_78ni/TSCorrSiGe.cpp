...
int main(int argc, char* argv[]){
   ...
   sprintf(gammafile,"../root/euroball/go4_%d.root",runG);
   sprintf(dssdfile,"../root/sistopper/ni78test/WASABI%04d.root",runD);

   TFile *rootfile = new TFile("root_dir/TSCorr_Si_Ge.root","RECREATE");
   Long64_t width=0;
   if(widt<200){
      width=widt;
   }else if(widt>=200){
   ...
   TSCorr->Draw();
   c1->SaveAs("pics/TSCorrSiGe.eps");
   c1->SaveAs("pics/TSCorrSiGe.root");
   rootfile->Write();
   rootfile->Close();
   cout<<"come here!!"<<endl;
   return 0;
