...
void TDC_offset_downstream() {
   ...
   file = new TFile("../root/sistopper/WASABI1032.root");
   foff = fopen("TDC_off_downstream.dat", "w");
   ...
   for (int i = 7; i <= 7; i++) {
      for (j = 0; j < 60; j=j+2) {
         // init histo
         hvqx[i][j] = new TH1F(hnamex[i][j].c_str(), hnamex[i][j].c_str(), 200, -5000, 5000);

         // scan data from ttree file
         char varexp[40];
         sprintf(varexp, "DSSD_T[%d][%d][0]", i, j);
         char selection[40];
         sprintf(selection, "dssd_X[0]==%d && dssd_E_X[%d][%d]>6000", j, i, j);
         ...
         if (fabs(meancond) > 0) {
            ...
            hbin1x[i][j] = hmaxx[i][j] - 100;
            hbin2x[i][j] = hmaxx[i][j] + 50;
      ...
      for (j = 0; j < 40; j++) {
         // init histo
         hvqy[i][j] = new TH1F(hnamey[i][j].c_str(), hnamey[i][j].c_str(), 200, -5000, 5000);

         // scan data from ttree file
         char varexp[40];
         sprintf(varexp, "DSSD_T[%d][%d][0]", i, j+60);
         char selection[40];
         sprintf(selection, "dssd_Y[0]==%d && dssd_E_Y[%d][%d]>4000", j, i, j);
         ...
         if (meancond != 0.0) {
            ...
            hbin1y[i][j] = hmaxy[i][j] - 100;
            hbin2y[i][j] = hmaxy[i][j] + 50;
