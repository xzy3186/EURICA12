...
class eurica_wasabi {
   public :
      ...
      virtual void     BetaGetPos(Int_t &mult, std::map <Int_t,vector <pair<Int_t,Int_t> > > &beta_event);
      /******** for sync tree **************/
...
void eurica_wasabi::BetaGetPos(Int_t &mult, std::map<Int_t,vector<pair<Int_t,Int_t> > > &beta_event)
{//mult is the multiplicity of beta event. map(z,vector(pair(x,y))) restore position
   ...
   for(Int_t z=0; z<NumDSSD; z++){
      ...
      for(Int_t x=1; x<NumStrX-1; x++){
         if(dssd_T_X[z][x][0]<3000 && dssd_T_X[z][x][0]>-3000 && dssd_E_X[z][x]>50){
            for(Int_t y=1; y<NumStrY-1; y++){
               if(dssd_T_Y[z][y][0]<3000 && dssd_T_Y[z][y][0]>-3000 && dssd_E_Y[z][y]>50){
                  mult++;
