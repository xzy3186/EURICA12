...
int main(int argc,char* argv[]){
   if(argc<2){
      cout << "USAGE: ./MakeWASABITree #runnumber" << endl;
      return 1;
   }
   ...
   sprintf(rootname,"../root/sistopper/TreeWASABI%04d.root",runN);
   ...
   TClonesArray * wasabi_array = new TClonesArray("TArtWASABI",1);
   wasabi_array->SetName("WASABI");
   new ((*wasabi_array)[0]) TArtWASABI(); // prepare WASABI data container
   tree->Branch(wasabi_array->GetName(),&wasabi_array);
   ...
   while(estore->GetNextEvent()){
      ...
      TArtWASABI *w = (TArtWASABI*)wasabi_array->At(0);

      rawevent->Clear();
      if(w) outtree.CopyToTArt(w); // cp outtree -> TArtWASABI
      tree->Fill();
      if(event%1000==0){
